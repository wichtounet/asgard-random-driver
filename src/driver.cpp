...
int main(){
    ...
    // Register the source
    auto nbytes = snprintf(write_buffer, buffer_size, "REG_SOURCE random");
    sendto(socket_fd, write_buffer, nbytes, 0, (struct sockaddr *) &server_address, sizeof(struct sockaddr_un));
    ...
    std::cout << "asgard:random: remote source: " << source_id << std::endl;

    // Register the sensor
    nbytes = snprintf(write_buffer, buffer_size, "REG_SENSOR %d %s %s", source_id, "RANDOM", "rand_100");
    sendto(socket_fd, write_buffer, nbytes, 0, (struct sockaddr *) &server_address, sizeof(struct sockaddr_un));
    ...
    std::cout << "asgard:random: remote sensor: " << sensor_id << std::endl;
